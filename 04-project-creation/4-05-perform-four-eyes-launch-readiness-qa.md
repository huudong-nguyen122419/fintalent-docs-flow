# 4.05 · Perform four-eyes launch-readiness QA

> **4. Project Creation** &nbsp;·&nbsp; flow not written yet

Review scope, taxonomy, screening, pricing, sourcing logic and client assumptions before launch.

## Who does it

| | SDR | KAM | Junior Ops | Senior Ops | Automation |
|---|---|---|---|---|---|
| **Short term** | · | · | S | R | · |
| **Mid-term** | · | S | S | R | S |

`R` primary owner &nbsp;·&nbsp; `S` supporting &nbsp;·&nbsp; `·` no routine part

## Automation

**Where it is heading.** Automated completeness and inconsistency checks

## Notes from the matrix

- **Comment.** Senior Ops is the independent quality gate.

---

*This page is a placeholder for the flow. When it is written it will follow the same shape as the SDR guide: the critical path first, the branches under the step they belong to, and a line on what each step is for.*
