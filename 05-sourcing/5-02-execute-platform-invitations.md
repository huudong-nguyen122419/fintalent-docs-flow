# 5.02 · Execute platform invitations

> **5. Sourcing**  ·  flow not written yet

Send targeted invitations and ensure the project reaches an adequate relevant talent pool.

## Who does it

|                | SDR | KAM | Junior Ops | Senior Ops | Automation |
| -------------- | --- | --- | ---------- | ---------- | ---------- |
| **Short term** | ·   | ·   | R          | ·          | ·          |
| **Mid-term**   | ·   | ·   | S          | ·          | R          |

`R` primary owner  ·  `S` supporting  ·  `·` no routine part

## Automation

**Where it is heading.** Automated invitation dispatch

## Notes from the matrix

* **Comment.** 80–100 invitations are indicative; candidate quality is the actual output.

***

_This page is a placeholder for the flow. When it is written it will follow the same shape as the SDR guide: the critical path first, the branches under the step they belong to, and a line on what each step is for._
