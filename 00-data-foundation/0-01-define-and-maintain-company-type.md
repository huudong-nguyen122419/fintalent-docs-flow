...
**For: all stakeholders.** One question each, yes/no. You only veto. **Silence = the recommended default executes after 48h.**
...
| **CPO** | Sign off the industry → type mapping for the five new types — Advisory Firm, Law Firm, Banks, Accounting Firm, Other (draft mapping at §3, G5)? | **Yes** — adopt the draft mapping; Corporate keeps only possible clients; the scenario list (7 after dropping S8) stays as confirmed. | The draft mapping becomes the rule behind fix card 1's backfill — reversible; amend it any time. |
| **COO** | Do the 2 symptom-check rows at the end of §2 match what support hears (incl. "my Corporate list is full of banks")? | **Yes** — rows confirmed by looking; the known-error playbook (§4) stays on demand. | Symptom rows stand as written; support can forward them as-is. |
| **CTO** | Accept the G5 default (industry → type mapping from provider industry data), with G4 held for your §3 answer? | **Yes** — G1 stands as accepted in rev 4 (G2 retired, G3 withdrawn — untyped-at-birth accepted); G5 is **reversible** (re-runnable backfill) and safe to default. | G5 executes with the default mapping; **G4 stays open — ONE-WAY, never defaulted.** |
| **Tech lead** | Need the technical appendix (surfaces + contract map) before pass 2 starts? | **Yes** — a full draft already exists (rev 3 of this audit); it is refreshed and attached before pass 2. | Appendix ships together with the pass-2 briefs. |

---

# 1 · Executive summary

**For: CEO.** *Business language, zero code terms, zero raw scores in prose.*
...
**1 · `QUICK WIN 1` `BLOCKER` `PRD GAP` — "Corporate" includes companies that can never be clients**

The tag is meant to mark possible Fintalent clients, but it is computed as merely *"not a sponsor, not a portfolio company"* — so every bank, M&A advisory and law firm imported from the provider lands in the prospect bucket, **while the provider's own industry data on each record says exactly what they are**. The fix: five new types — Advisory Firm, Law Firm, Banks, Accounting Firm, Other — driven by that industry data, leaving Corporate = possible clients only.

*Cost of delay:* every Corporate-filtered target list and report keeps over-counting; SDR hours burn on firms that can never buy — against ~2–3 days of work once the industry → type mapping is signed (48h default).

**2 · `QUICK WIN 2` `BLOCKER` — Nobody can fix a wrong company type**
...
*Cost of delay:* every day, outreach and reports keep counting misclassified companies — wasted SDR effort and eroding trust in the data, against roughly 1–3 days of work.

**3 · `QUICK WIN 3` `CRITICAL` — Three-plus competing notions of "company type" coexist**
...
*Cost of delay:* every cross-team report keeps reconciling mismatched numbers by hand, and decisions risk resting on the wrong ones.

**4 · `QUICK WIN 4` `MAJOR` — Bulk edits leave no audit trail**

Individual edits are recorded and re-scored automatically; bulk edits re-score but skip the record of who changed what.

*Cost of delay:* *"who changed this?"* stays unanswerable for bulk actions — an audit risk that compounds with every one.
...
**For: COO + CPO.** *The flow as a person walks it. Plain words — read this aloud in standup.*
...
*Answers the COO's §0 row by looking, not memory. Support can forward this as-is.*
...
**For: CTO.** *Change contracts, not implementations. Every change is tagged `DECIDED` or `NEEDS CTO` and traced to a pain ID. JSON samples live only here. Exact file evidence: technical appendix (rev 3).*
...
### 3A-E · Completion checklist *(verification only)*
...
### 3B-D · Completion checklist *(verification only)*
...
**Acceptance:** *Given* a provider company that is neither Sponsor nor Portfolio and whose industry maps to a service type (e.g. an M&A advisory) · *When* the backfill or a fresh provider refresh runs · *Then* it is tagged Advisory Firm, leaves the Corporate filter and appears under the new chip, while a client-industry peer keeps its Corporate tag.
...
**Acceptance:** *Given* an admin with company-edit rights viewing a company tagged Corporate · *When* they set Sponsor on and save · *Then* the badges update instantly, the company's history lists the change with their name, and a reload still shows Sponsor.
...
**Acceptance:** *Given* a company whose legacy text type disagrees with its official tag · *When* an admin views the list, panel or a report · *Then* only the official tag drives what is shown and counted, and the legacy text no longer appears.
...
**Acceptance:** *Given* an admin bulk-assigns a bucket to 20 companies · *When* the write completes · *Then* each of the 20 histories shows the change with the admin's name and timestamp.
...
*Removed from scope by product (2026-08-11): **Protect manual types from refresh** (P3 · RPN 50) and **Reconnect the refresh button** (P2 · RPN 24) — both pains stay on the register; their full cards are preserved in rev 4. Rev 4's **Enforce lifecycle validation** card (P10 · RPN 16 · E1) also sits in the pass-2 backlog, unchanged.*
